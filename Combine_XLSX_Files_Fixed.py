...
from Extra import fixed_directory, fixed_data_columns, fixed_number_format_columns
...
column_count = 0
for column_header in fixed_data_columns:
...
for row_dict in frontier_dict:
    ...
    try:
        ...
        continue
    for column in range(len(fixed_data_columns)):
        ...
        cell_format.set_align('center')
        cell_format.set_align('vcenter')
        header_key = fixed_data_columns[column_count]
        if header_key in fixed_number_format_columns:
            ...
            try:
                ...
                try:
                    ...
                    if row_dict['Country'] + ' ' + row_dict[
                        'Operator'] + ': Cannot find column with title --> ' + header_key not in dict_key_errors:
                        dict_key_errors.append(
                            row_dict['Country'] + ' ' + row_dict['Operator'] +
            ...
            try:
                ...
                worksheet.write(row, column_count, '', cell_format)
                if row_dict['Country'] + ' ' + row_dict['Operator'] + ': Cannot find column with title --> ' + header_key not in dict_key_errors:
                    dict_key_errors.append(row_dict['Country'] + ' ' + row_dict['Operator'] +
                                           ': Cannot find column with title --> ' + header_key)
                    # print(row_dict)
        column_count += 1
    row += 1
